import { Divider, Drawer, IconButton, List, ListItem, ListItemButton, ListItemButtonProps, ListItemIcon, ListItemText, useTheme } from "@mui/material"
...
function MyListItemButtonLink({children, to, ...props}: PropsWithChildren<MyListItemButtonLinkProps>) {
  return (         
...
function Sidebar(props: SidebarProps) {
  ...
  return (
    <>
      ...
      >
        <DrawerHeader>
          <IconButton onClick={props.handler} sx={{ color: ColorConfig.sidebar.color }}>
        ...
        <List>
          {['日付相互変換'].map((text, index) => (
            <ListItem key={text} disablePadding>
              <MyListItemButtonLink to="/datetime" sx={{
                "&: hover": {
                ...
                <ListItemIcon sx={{ color: ColorConfig.sidebar.color}}>
                  {index % 2 === 0 ? <ManageHistoryIcon /> : <ManageHistoryIcon />}
                </ListItemIcon>
